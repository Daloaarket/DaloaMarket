...
const ListingCard: React.FC<ListingCardProps> = ({ 
  ...
  return (
    <Link 
      ...
      className="group block bg-white rounded-xl lg:rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-grey-100 hover:border-primary-200 transform hover:-translate-y-1"
    >
      {/* Image Container */}
      ...
      <div className="p-3 lg:p-4 space-y-2 lg:space-y-3">
        ...
        <div className="space-y-1 lg:space-y-2">
          <h3 className="font-semibold text-grey-900 line-clamp-2 text-sm lg:text-base leading-tight group-hover:text-primary transition-colors">
            {listing.title}
          </h3>
        ...
        {/* Location and Date */}
        <div className="flex items-center justify-between text-xs lg:text-sm text-grey-600">
          <div className="flex items-center min-w-0 flex-1">
            <MapPin className="h-3 w-3 lg:h-4 lg:w-4 mr-1 text-grey-400 flex-shrink-0" />
            <span className="truncate">{listing.district}</span>
          </div>
          <div className="flex items-center ml-2 flex-shrink-0">
            <Clock className="h-3 w-3 lg:h-4 lg:w-4 mr-1 text-grey-400" />
        ...
        {sellerName && (
          <div className="flex items-center justify-between pt-2 border-t border-grey-100">
            <span className="text-xs lg:text-sm text-grey-600 truncate flex-1">{sellerName}</span>
            {sellerRating && (
              <div className="flex items-center ml-2 flex-shrink-0">
                <Star className="h-3 w-3 lg:h-4 lg:w-4 text-primary fill-current" />
                <span className="text-xs lg:text-sm font-medium ml-1">{sellerRating.toFixed(1)}</span>
              </div>
            )}
          </div>
        )}
